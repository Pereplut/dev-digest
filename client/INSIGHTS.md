...
### 2026-09-24 — [dep] Suspense is unavailable for a query that needs `enabled`, so an in-component loading branch is the only option
**Context:** Smart Diff's Files changed tab renders a placeholder while `useSmartDiff` resolves; reviewing it raised the obvious App Router question — why not a Suspense boundary or `loading.tsx`?
**Insight:** neither is available here, and the reason is not stylistic. `loading.tsx` is a route-segment boundary covering the segment's initial SERVER render, so it cannot represent a client query settling after mount and would blank the whole PR-detail route, tabs included. A React `<Suspense>` only suspends on a thrown promise, which `useQuery` never throws — only `useSuspenseQuery` does, and **`useSuspenseQuery` has no `enabled` option**. Adopting it would silently drop `useSmartDiff`'s `enabled: !!prId`, which exists because `prId` appears only after the PR-detail query resolves and THAT request is what persists `pr_files` server-side — so the smart-diff call would race an empty table.
**Apply:** inside an already-client route, a query that needs `enabled` gets an in-component `if (loading) return <placeholder>` branch; don't "modernise" it into Suspense. Gate on `isLoading`, not `isFetching`: in TanStack v5 `isLoading = isPending && isFetching`, so a disabled query reports false (no stall), an errored query reports false (falls through to the fallback branch), and a background refetch reports false (no placeholder flashing over content being read).
**Evidence:** `client/src/lib/hooks/core.ts:183` (`enabled: !!prId` and why), `client/src/app/repos/[repoId]/pulls/[number]/_components/DiffTab/DiffTab.tsx` (the `awaitingGroups` branch); `client/node_modules/@tanstack/query-core/build/modern/queryObserver.js:307-310` (`isLoading = isPending && isFetching`, query-core 5.101.0).
