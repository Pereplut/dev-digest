...
### 2026-09-23 — [odd] `ReviewOutcome.assembly` describes a prompt map-reduce never sent
**Context:** adding prompt-composition logging (spec 0009); the obvious implementation reads `outcome.assembly` on the server, where a logger and tokenizer already exist.
**Insight:** `assembly` is seeded with a **whole-diff** assembly and overwritten only `if (mode === 'single-pass')`. On a map-reduce run the real prompts are assembled per file inside the chunk loop, so `outcome.assembly` holds text that reached no model, and one record stands for N LLM calls — a plausible, wrong answer on exactly the large PRs where the question gets asked.
**Apply:** treat `outcome.assembly` as a single-pass-only trace approximation. For what was actually sent, emit per call from inside the loop (the `onPromptAssembled` sink); assert per-chunk diff sizes *differ*, since a run-level record passes any per-run assertion.
**Evidence:** `reviewer-core/src/review/run.ts:178` (the seed), `:209` (the conditional overwrite), `:208` (the per-chunk assembly); regression test `reviewer-core/test/prompt-assembly-log.test.ts` ("gives each map-reduce call its own diff size").
