...
/**
 * F1 — settings module. Transport layer only: delegates to SettingsService.
 ...
 *
 * Secrets are NOT stored here — only non-secret prefs.
 */
export default async function settingsRoutes(appBase: FastifyInstance) {
  ...
  app.get('/settings', async (req): Promise<Settings> => {
    const { workspaceId } = await getContext(app.container, req);
    return service.get(workspaceId);
  });

  app.get('/settings/secrets-status', async (req): Promise<SecretsStatus> => {
    // Called for its tenancy check: the status is workspace-independent, but
    // the endpoint still requires a resolved context.
    await getContext(app.container, req);
    return service.secretsStatus();
  });

  app.put('/settings', { schema: { body: SettingsUpdate } }, async (req): Promise<Settings> => {
