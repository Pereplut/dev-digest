...
/** Upper bound on one agent's skill links (each one lands in every prompt). */
const MAX_AGENT_SKILLS = 100;

/** Full ordered replacement of an agent's skill links; array order = prompt order. */
const SetSkillsBody = z.object({
  skills: z
    ...
    .refine((links) => new Set(links.map((l) => l.skill_id)).size === links.length, {
      message: 'Each skill may appear only once',
    }),
});
...
export default async function agentsRoutes(appBase: FastifyInstance) {
  ...
  app.put(
    ...
    async (req) => {
      const { workspaceId } = await getContext(app.container, req);
      return service.setSkillLinks(workspaceId, req.params.id, req.body.skills);
    },
  );
