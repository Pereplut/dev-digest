...
**A structural fact worth keeping:** a raw-text pre-filter can never be as wide as a check over de-quoted tokens, so it must never be part of the guard. The hook's filter dropped `--out\put=` (fixed by widening it from `--output` to `\bgit\b`) and **still drops a quoted program name today**: `gi"t" diff --output=x` has no literal `git` for the regex, so it never reaches the check — which does catch it, since shlex de-quotes first. That gap is pinned by a test rather than fixed, and named in the hook's docstring.
...
**Evidence:** `.claude/hooks/pr-self-review-gate.py` (`MAYBE_GATED`, and the deny that runs ahead of the review-state check); `review_scope.py` `write_escape()` + `_segments()` + `_program_and_args()`; `test_review_scope.py` `WriteEscapeTest` — 14 escape shapes, 8 clean, one test asserting the pre-filter matches every escape in the corpus, and one pinning the shapes it is known to lose. Probes: before the fix, `--out\put=` and `$(echo --output=…)` each created a 328-byte file unblocked; after it, both are denied. `gi"t" …` still writes, by the pre-filter gap above.

### 2026-09-23 — [fix] `Bash(git diff:*)` reads any file on disk, and the `deny` list cannot stop it
**Context:** a `/pr-self-review` security finding on my own permissions commit; I reproduced it before accepting it.
**Insight:** an allowlist entry matches a command PREFIX, so `Bash(git diff:*)` approves every flag after it — including `--no-index`, which diffs two arbitrary paths and prints them. `git diff --no-index <a file holding SECRET_TOKEN=abc123> /dev/null` printed the line with no approval prompt, straight past `Read(./**/.env)`, `Read(~/.ssh/**)` and the rest: a `deny` entry cannot reach a flag that trails the subcommand. The same commit had already closed the WRITE escape of the very same flag family (`--output`) and missed the read one, because I went looking for writes.
**Apply:** when allowlisting a "read-only" command, enumerate what its flags can do in BOTH directions before trusting the label — and put the check on de-quoted tokens in a PreToolUse hook, since no `deny` pattern can express "this flag, after this subcommand". Extends: "A Bash allowlist entry matches the literal command, and a compound is checked part by part".
**Evidence:** `.claude/skills/pr-self-review/scripts/review_scope.py` (`git_escape`, now both flags), `.claude/hooks/pr-self-review-gate.py:34` (the pre-filter widened to the flags themselves); the same command is denied end-to-end after the fix, verified by re-running it.

### 2026-09-23 — [fix] git enters no-index mode with no flag, so matching the flag spelling guarded nothing
**Context:** the fix for the entry above denied the `--no-index` token; the next review round found the hole was still open and I reproduced it before believing it.
**Insight:** git-diff(1) says the flag may be omitted "when at least one of the paths points outside the working tree", and it is true of `/dev/null` too — measured with git 2.43.0 from the repo root, `git diff <a tmp file holding a fake secret> /dev/null` printed the secret with no flag anywhere in the command and no approval prompt, while the same command spelled with `--no-index` was denied. A guard written against a spelling tests the spelling; the capability is what the allowlist actually hands over. The token check now also rejects any `git diff` operand that is absolute, holds a `..` segment, or is `/dev/null`.
**Apply:** when a guard covers a tool's flag, read that flag's own documentation for when the tool turns the behaviour on by itself, and write the test corpus from the capability, not from the flag. A corpus that contains only commands with the literal flag will pass at full green while the hole is wide open — this one did.
**Evidence:** `.claude/skills/pr-self-review/scripts/review_scope.py` (`_outside_tree_operand`), `IMPLICIT_READ_ESCAPES` in `test_review_scope.py`; denied end-to-end after the fix, and 16 real git commands (including `git -C server diff --stat` and `git commit -F /abs/path`) still pass. Extends: "`Bash(git diff:*)` reads any file on disk, and the `deny` list cannot stop it".

### 2026-09-23 — [fix] A token guard cannot see what the shell will do to the token
**Context:** third and fourth attempts at the same `Bash(git diff:*)` read escape; an adversarial review round broke each one and I reproduced every break before fixing it.
**Insight:** three distinct holes, all from inspecting the command as WRITTEN rather than as RUN. (1) `shlex` de-quotes but never expands, so `git diff $HOME/.ssh/id_rsa .gitignore` is one token with no leading `/`, no `~`, no `..` — the shell makes it absolute afterwards; executed live, it printed 121 lines of `~/.bashrc`. (2) Skipping the argument of `-C` was wrong AFTER the subcommand, where `-C` is `--find-copies` and takes none, so the skip swallowed the path: `git diff -C /etc/hostname .gitignore` printed it. (3) Options BEFORE the subcommand were never looked at, and `git -C / diff etc/hostname etc/hosts` relocates git so both operands look relative. Meanwhile the hook's raw-text pre-filter was narrower than the guard for the third time running, letting `gi"t" diff /etc/passwd /dev/null` past before the check could run.
**Apply:** a guard that cannot evaluate an operand must treat it as hostile — deny on `$` or a backtick rather than trying to reason about it; never skip a token to reach another, because the skip is where the payload hides; and check the tool's options on BOTH sides of its subcommand, since the same spelling means different things there. Widen the cheap pre-filter every time the real check gets wider, or it becomes the hole.
**Evidence:** `.claude/skills/pr-self-review/scripts/review_scope.py` (`_reaches_outside`, `_outside_tree_operand`); `IMPLICIT_READ_ESCAPES` in `test_review_scope.py` now carries all six shapes; `.claude/hooks/pr-self-review-gate.py:45` (`\bdiff\b` added). All denied end-to-end after the fix; 16 real git commands still pass. Extends: "git enters no-index mode with no flag, so matching the flag spelling guarded nothing".

### 2026-09-23 — [measured] Dropping the gate's raw-text pre-filter costs 19 ms on every Bash call
**Context:** the pre-filter in front of `git_escape` was narrower than the check four rounds running, so it was removed rather than widened a fifth time; the docstring first claimed the cost was "a few milliseconds", which was a guess.
**Insight:** measured on this machine, median of 15 runs, feeding the hook its real PreToolUse JSON: **41.1 ms** per Bash command with the check always running, **22.4 ms** with the old short-circuit, and **16.6 ms** for a bare `python3 -c pass`. So the guard costs ~19 ms on top of ~17 ms of interpreter startup that was paid either way — on every single Bash call, not just git ones. Worth it against a silent arbitrary-file read, but it is a real tax and the docstring now states the number rather than an adjective.
**Apply:** a regex over un-lexed text can never be as wide as a check over de-quoted tokens, so never put one in FRONT of such a check as an optimisation — it becomes the hole, four times here. If the cost must come back, the filter has to be provably no narrower than the check.
**Evidence:** `.claude/hooks/pr-self-review-gate.py` (no pre-filter; the timing note is in its module docstring); `test_the_hook_denies_every_escape_however_it_is_spelled` in `test_review_scope.py` now drives the real hook per command, which is why that suite went from 0.55 s to 2.7 s.

### 2026-09-23 — [fix] `git blame --contents` and `wc --files0-from` print any file the caller names
**Context:** round six of the same allowlist review; both were reproduced on a throwaway file before being believed.
**Insight:** `git blame --contents=<file> HEAD -- <tracked path>` prints every line of `<file>` attributed to "External file", and `wc --files0-from=<file>` reads the file and echoes its bytes back inside its own error message (`wc: 'FAKE_A=1111'$'\n'…: No such file or directory`). Both were auto-approved by `Bash(git blame:*)` and `Bash(wc:*)`. The guard missed them because it was keyed on git's `diff` subcommand; it is now a `{program: flags}` table covering `--output`, `--no-index`, `--contents`, `--extcmd` and `--files0-from`, and `difftool` is treated as `diff`. `Bash(wc:*)` was dropped from the allowlist as well.
**Apply:** "it only reads things" is a statement about intent, not capability — the same trap INSIGHTS already records for `find -exec` and `rg --pre`. Before allowlisting a command, look for the flag that takes a FILENAME and reports what is in it; error messages count as output.
**Evidence:** `.claude/skills/pr-self-review/scripts/review_scope.py` (`_ESCAPE_FLAGS`), `OTHER_PROGRAM_ESCAPES` in `test_review_scope.py`, `.claude/settings.json` (no `Bash(wc:*)`); both denied end-to-end after the fix.

### 2026-09-23 — [fix] Seven rounds of blacklisting flags lost; narrowing the allowlist was the fix
**Context:** `Bash(git diff:*)` and `Bash(git blame:*)` were on the permission allowlist, and a PreToolUse hook tried to subtract the dangerous spellings from what those prefixes granted.
**Insight:** it never converged. Nine distinct bypasses over seven review rounds, each reproduced on disk, each "fixed", each followed by a new one: `--output`, `--no-index`, implicit no-index from an outside operand, `$HOME/…` (shlex de-quotes but never expands), a bogus `-C` argument skip, `git -C /` relocation, `git blame --contents`, `wc --files0-from`, `--cont=` (git's parse-options takes any unambiguous abbreviation), `{a,b}` brace expansion, `git blame -S` (dumps a file through `error: bad graft data:`). The attack surface is the product of three open-ended sets — every file-naming flag of every allowed subcommand, every shell expansion the lexer does not perform, every abbreviation of every flag — and a blacklist cannot be built from that product. The prefix grant was the bug; the hook was subtracting from an infinite set.
**Apply:** `Bash(<cmd>:*)` grants everything that can follow, so never allowlist a prefix whose command takes flags you have not enumerated — and you cannot enumerate them. Allowlist exact invocations, or leave the command to prompt and keep the hook as defence in depth, which is what this repo now does for `git diff` and `git blame`. A guard that keeps needing a new rule per round is answering the wrong question.
**Evidence:** `.claude/settings.json` (no `Bash(git diff:*)`, `Bash(git blame:*)`, `Bash(wc:*)`); `.claude/skills/pr-self-review/scripts/review_scope.py` (`_ESCAPE_FLAGS` with abbreviation-prefix matching, brace refusal, and a fallback regex built FROM the table rather than hand-listed); 39 tests in `test_review_scope.py`, three of which drive the real hook.

### 2026-09-24 — [tool] A `/pr-self-review` run that stops before `write-verdict` leaves no trace at all
**Context:** the previous session was told three times that the review had been re-run and passed; `verdict.json` was byte-identical each time.
**Insight:** only the `write-verdict` subcommand touches the state, and the gate blocks every other writer, so a run that dies earlier (interrupted, a review agent failing) leaves the PREVIOUS verdict in place looking valid — same `status: pass`, same fingerprint, same mtime. There is no partial write and no error file, so "I ran it" and "it wrote a verdict" are independent facts. Measured: `verdict.json` mtime stayed `22:43:53` with `head c0173a2143…` across three reported re-runs while `HEAD` was `1a912238c4…`.
**Apply:** never take a reported PASS as the gate's state. Run `python3 .claude/skills/pr-self-review/scripts/review_scope.py check` and compare the verdict's `head` with `git rev-parse HEAD`; an unchanged mtime means the skill never reached its last step, not that the diff is clean. Ask for the run's `fingerprint:` line rather than re-reading the same file.
**Evidence:** `.claude/skills/pr-self-review/scripts/review_scope.py:33` (`STATE_REL`, one canonical path resolved from the git root), `:947` (`write-verdict` is the only writer), `:555` (`check`); `check` reported `the last review is stale — the diff changed after it ran (2026-09-23T20:43:53+00:00)` three times unchanged.
